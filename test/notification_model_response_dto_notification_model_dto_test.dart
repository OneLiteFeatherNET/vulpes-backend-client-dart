import 'package:test/test.dart';
import 'package:vulpes_backend_client/vulpes_backend_client.dart';

// tests for NotificationModelResponseDTONotificationModelDTO
void main() {
  final instance = NotificationModelResponseDTONotificationModelDTOBuilder();
  // TODO add properties to the builder and call build()

  group(NotificationModelResponseDTONotificationModelDTO, () {
    // The id of the model
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // Model Name for the UI
    // String uiName
    test('to test the property `uiName`', () async {
      // TODO
    });

    // Variable name for the generation
    // String variableName
    test('to test the property `variableName`', () async {
      // TODO
    });

    // Description of the Notification
    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // Material type of the Notification
    // String material
    test('to test the property `material`', () async {
      // TODO
    });

    // Frame type of the Notification
    // String frameType
    test('to test the property `frameType`', () async {
      // TODO
    });

    // Title of the Notification
    // String title
    test('to test the property `title`', () async {
      // TODO
    });

  });
}
