import 'package:test/test.dart';
import 'package:vulpes_client/vulpes_client.dart';

// tests for ItemModelResponseDTOItemModelEnchantmentResponseDTO
void main() {
  final instance = ItemModelResponseDTOItemModelEnchantmentResponseDTOBuilder();
  // TODO add properties to the builder and call build()

  group(ItemModelResponseDTOItemModelEnchantmentResponseDTO, () {
    // UUID of the Item Model
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // Map which contains the enchantments as string and short
    // BuiltMap<String, int> enchantments
    test('to test the property `enchantments`', () async {
      // TODO
    });

  });
}
